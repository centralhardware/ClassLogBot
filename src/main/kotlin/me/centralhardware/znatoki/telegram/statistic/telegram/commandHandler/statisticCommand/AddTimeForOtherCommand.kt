...
fun BehaviourContext.addTimeForOtherCommand() = onCommand(Regex("addTimeForOther|addtimeforother")) {
    if (!ensureNoActiveFsm(it.userId())) {
        return@onCommand
    }

    Storage.create(it.userId(), startTimeFsm(it, canAddForOthers = true))
}
