...
@Component
class PaymentRestoreCallback(private val paymentMapper: PaymentMapper,
    ...
    override fun handle(callbackQuery: CallbackQuery, from: User, data: String) {
        ...
        val id = data.replace("paymentRestore-", "").toInt()
        ...
        val editMessageReplyMarkup = EditMessageReplyMarkup
            ...
            .replyMarkup(inlineKeyboard {
                row { btn("удалить", "paymentDelete-${id}") }
            }.buildReplyMarkup())
            .build()
        sender.send(editMessageReplyMarkup)
    }

    override fun isAcceptable(data: String): Boolean = data.startsWith("paymentRestore-")
}