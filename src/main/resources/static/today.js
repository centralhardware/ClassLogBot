...
async function loadTodayData() {
    try {
        const [lessonsRes, paymentsRes] = await Promise.all([
            fetch('/api/lessons', { headers: { 'Authorization': `tma ${tg.initData}` } }),
            fetch('/api/payments', { headers: { 'Authorization': `tma ${tg.initData}` } })
        ]);
...
async function saveLessonChanges() {
    ...
    try {
        ...
        const updateResponse = await fetch(`/api/lessons/${currentLessonId}`, {
            method: 'PUT',
            headers: {
                'Authorization': `tma ${tg.initData}`,
        ...
        for (const studentId of lessonStudentChanges.toRemove) {
            const removeResponse = await fetch(`/api/lessons/${currentLessonId}/student/${studentId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `tma ${tg.initData}`
                }
        ...
        for (const {studentId} of lessonStudentChanges.toAdd) {
            const addResponse = await fetch(`/api/lessons/${currentLessonId}/student`, {
                method: 'POST',
                headers: {
                    'Authorization': `tma ${tg.initData}`,
                    'Content-Type': 'application/json'
