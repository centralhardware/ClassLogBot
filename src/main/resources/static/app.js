...
function showAccessDenied() {
    document.body.innerHTML = `
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; padding: 20px; text-align: center; background: var(--tg-theme-bg-color, #fff); color: var(--tg-theme-text-color, #000);">
            ...
            <h1 style="font-size: 24px; margin-bottom: 10px;">Доступ запрещен</h1>
            <p style="font-size: 16px; color: var(--tg-theme-hint-color, #999);">У вас нет доступа к веб-интерфейсу</p>
        </div>
    `;
}
...
function setupEventListeners() {
    ...
    if (isAdmin) {
        document.getElementById('stats-tutor-select').addEventListener('change', loadStatistics);
    }
...
function switchPage(pageName) {
    ...
    if (pageName === 'today' && todayLessons.length === 0) {
        ...
        // Load report if subject is selected
        if (subjectSelect.value && !currentReport) {
            loadReport();
        }
